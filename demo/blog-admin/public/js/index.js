...
layui.define(['layer', 'laytpl', 'form', 'upload', 'util' , 'layedit'], function(exports){
  ...
  if(device.ie && device.ie < 8){
    layer.alert('如果您非得使用ie浏览Node后台管理系统，那么请使用ie8+');
  }
  ...
  //缩略图上传
  layui.upload({
    url: '/aliadmin/article/thumbnmil',
    title : '上传文章缩略图',
    success: function(res){
      layer.closeAll('loading');
      if(res.type=='success'){
          $('#head').attr('src','../../'+res.data.src);
          $('[name="thumbnail"]').val(res.data.src);
  ...
  layedit.set({
    uploadImage: {
      url: '/aliadmin/article/contentImg'
      ,type: 'post'
    }
  });
  //配置编辑器
  var editContent = layedit.build('LayEdit',{
      height: 200
  }); 

  //表单验证
  form.verify({
      textarea : function (value,obj) {
  ...
  form.on('submit(sub)', function(data){
    ...
    $.post(action, data.field, function(res){
      if(res.type=='success'){
          layer.msg(res.content,{time:1800});
          setTimeout(function(){
              window.location = '/aliadmin/article'
          },500)
      }else{
          layer.msg(res.content,{time:1000});
      }
